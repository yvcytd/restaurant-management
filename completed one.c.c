...
struct node *headc = NULL,*newnode,*tailc = NULL;// head and tail pointers of customer
...
struct node* createadmin(struct node *head,int data, char foodname[25], float price)
{
    ...
    if(temp==NULL) // head and tail assign new node
        heada = taila = newnode;
    else
    {
...
// this function creates a node for costumer
...
// display list function which takes head pointer as argument.
...
// this function calculates the total sales of the day
...
// this function performs the task of deleting the data from the list whose respective head pointer is passed.
...
// this function performs the task of deleting food items from the admin
int deleteadmin()
{
...
// this is a function to delete a node of customer
int deletecustomer()
{
    printf("\n\t\t\t\t\tEnter serial no. of the food item which is to be deleted: ");
    int num;
...
// display the total bill of food items ordered by the customer
void displaybill()
{
FILE *fp;
fp= fopen("Total sales.txt","w");
if ("fp==null"){
...
fprintf(fp, "the total bill is %f",total_price);
fclose(fp);

    printf("\t\t\t\t\t\t\tTotal price: %0.02f\n",total_price);

}
// this performs the task of deleting the entire list
...
void admin() // this function opens up the admin section and provides its interface and functionalities

{system("cls"); 
		char username[10]="yukesh",pak[10]="admin";
 printf("\t\t\t\t*****enter the username*****\n\n\nusername:");
   scanf("%s",&username);
    printf("\t\t\t\t*****enter the password*****\n\n\npassword:");
   scanf("%s",&pak);
   if (strcmp(username,"yukesh")==0 && strcmp(pak,"admin")==0)
   {system("cls");
    ...
    while(1)
    {
        ...
        int opt;
        scanf("%d",&opt);// scan choice of user
...
// from here the actual program execuation begins
int main()
{// here i have added food menu with 5 dishes 
